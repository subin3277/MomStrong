...
enum ButtonsState_second{
    GONE, LEFT_VISIBLE, RIGHT_VISIBLE
}
public class ItemTouchHelperCallback_second extends ItemTouchHelper.Callback{
    private ItemTouchHelperListener listener;
    private boolean swipeBack = false;
    private ButtonsState_second buttonsShowedState = ButtonsState_second.GONE;
    private static final float buttonWidth = 115;
    private RectF buttonInstance = null;
    private RecyclerView.ViewHolder currenrtItemViewHolder = null;
    public ItemTouchHelperCallback_second(ItemTouchHelperListener listener) {
    ...
    public void onChildDraw(@NonNull Canvas c, @NonNull RecyclerView recyclerView,
        ...
        if(actionState == ItemTouchHelper.ACTION_STATE_SWIPE){
            if(buttonsShowedState != ButtonsState_second.GONE){
                if(buttonsShowedState == ButtonsState_second.LEFT_VISIBLE)
                    dX = Math.max(dX, buttonWidth);
                if(buttonsShowedState == ButtonsState_second.RIGHT_VISIBLE)
    ...
    private void setTouchListener(final Canvas c, final RecyclerView recyclerView,
        ...
        recyclerView.setOnTouchListener(new View.OnTouchListener() {
            ...
            public boolean onTouch(View v, MotionEvent event) {
                ...
                if(swipeBack){ if(dX < -buttonWidth) buttonsShowedState = ButtonsState_second.RIGHT_VISIBLE;
                else if(dX > buttonWidth) buttonsShowedState = ButtonsState_second.LEFT_VISIBLE;
                    if(buttonsShowedState != ButtonsState_second.GONE){
    ...
    private void setTouchUpListener(final Canvas c, final RecyclerView recyclerView ,
        ...
        recyclerView.setOnTouchListener(new View.OnTouchListener() {
            @Override
            public boolean onTouch(View v, MotionEvent event) {
                ItemTouchHelperCallback_second.super.onChildDraw(c, recyclerView, viewHolder,
